@echo off
REM ========================================
REM SCRIPT TU DONG HOAN TOAN - CHAY UI TESTS
REM ========================================
REM Script nay chay hoan toan tu dong:
REM   - Khong can pause hay tuong tac
REM   - Tu dong retry neu co loi
REM   - Tu dong tao bao cao HTML
REM   - Tu dong mo HTML report khi xong
REM ========================================
...
echo ========================================
echo.
echo Script se tu dong chay tat ca 74+ UI test cases
echo va tao bao cao HTML day du.
echo.
...
REM ========================================
REM STEP 1: Check device (auto-retry)
REM ========================================
echo ========================================
...
REM Robust device detection: log devices and proceed even if detection is flaky
set RETRY_COUNT=0
:CHECK_DEVICE
set DEVICE_FOUND=0
set DEVICE_ID=
...
REM Log current device list for diagnostics
echo [ADB DEVICES SNAPSHOT] > "%REPORT_DIR%\logs\devices.txt"
adb devices >> "%REPORT_DIR%\logs\devices.txt" 2>&1

REM Try fast check with get-state
set STATE=
if defined TARGET_DEVICE (
    for /f "delims=" %%S in ('adb -s !TARGET_DEVICE! get-state') do set STATE=%%S
    if /I "!STATE!"=="device" (
        set DEVICE_FOUND=1
        set DEVICE_ID=!TARGET_DEVICE!
    )
) else (
...
REM Fallback: parse adb devices table
...
if !DEVICE_FOUND! EQU 0 (
    ...
    if !RETRY_COUNT! LSS 5 (
        ...
        echo [WARNING] Device detection failed after 5 retries. Continuing anyway...
        echo See snapshot: %REPORT_DIR%\logs\devices.txt
        echo.
    )
) else (
...
REM ========================================
REM STEP 2: Clean and build (auto-retry)
REM ========================================
echo ========================================
...
REM Build with auto-retry
set BUILD_RESULT=1
set BUILD_RETRY=0
:BUILD_RETRY
set /a BUILD_RETRY+=1
echo Building test APK (attempt !BUILD_RETRY!/3)...
...
REM Force Gradle to finish writing reports
...
REM ========================================
REM STEP 4: Copy HTML reports (auto-retry)
REM ========================================
echo ========================================
...
if exist "app\build\reports\androidTests\connected\debug" (
    ...
    xcopy /E /I /Y "app\build\reports\androidTests\connected\debug" "%REPORT_DIR%\reports\" 2>&1 | findstr /V /C:"File(s) copied" | findstr /V /C:"Dir(s)"
    
    REM Wait a bit for file system to sync
    timeout /t 2 /nobreak >nul
...
REM ========================================
REM STEP 5: Copy XML test results
REM ========================================
echo ========================================
...
REM Additional: copy UTP logs and device snapshot for easier debugging
if exist "%REPORT_DIR%\logs" (
    if exist "app\build\outputs\androidTest-results\connected\debug" (
        xcopy /E /I /Y /Q "app\build\outputs\androidTest-results\connected\debug" "%REPORT_DIR%\logs\utp_debug\" >nul 2>&1
        echo [INFO] Copied UTP debug logs to %REPORT_DIR%\logs\utp_debug\
        echo.
    )
)
...
REM ========================================
REM STEP 7: Create summary report
REM ========================================
echo ========================================
...
REM ========================================
REM STEP 8: Final verification and auto-open
REM ========================================
echo ========================================
