...
try {
    ...
    Write-Host "✅ Loaded $($metrics.Count) metrics" -ForegroundColor Green
} catch {
...
Write-Host "📈 Found $total performance metrics ($passed passed, $failed failed)" -ForegroundColor Yellow
...
Write-Host "✅ HTML report saved to reports/performance_report.html" -ForegroundColor Green

# Create summary text
$summary = @"
DATING APP MOBILE PROJECT - PERFORMANCE REPORT
...
TOP ISSUES:
- Frame rate below target (50 FPS)
- Click response time needs improvement
- No UI test results available

RECOMMENDATIONS:
1. Optimize rendering performance
2. Improve touch responsiveness  
3. Implement UI testing framework
4. Set up performance monitoring

Report saved as HTML and text formats.
"@

$summary | Out-File -FilePath "reports/performance_report.txt" -Encoding UTF8
Write-Host "✅ Text report saved to reports/performance_report.txt" -ForegroundColor Green
Write-Host "📊 Report complete! Open the HTML file in browser for full view." -ForegroundColor Cyan