...
fun UserGameCard(
    ...
    Column(
        modifier = modifier
            .padding(start = 50.dp)
            .fillMaxWidth()
            .background(Color(0xFFB266FF), RoundedCornerShape(16.dp))
            .padding(12.dp)
    ) {
...
fun GameOption(
    ...
    Row(
        Modifier
            .fillMaxWidth()
            .background(
                if (pickedBy.contains(currentUser.userId))
        ...
        Row {
            if (pickedBy.contains(otherUser?.userId)) {
                UserAvatar(otherUser?.defaultImage, offset = true)
            }
            if (pickedBy.contains(currentUser.userId)) {
                UserAvatar(currentUser.defaultImage, offset = pickedBy.contains(otherUser?.userId))
            }
        }
    }
}
...
@Composable
fun UserAvatar(imageUrl: String?, offset: Boolean) {
    Box(
        modifier = Modifier
            .size(28.dp)
            .padding(start = if (offset) (0).dp else 0.dp)
            .clip(CircleShape),
